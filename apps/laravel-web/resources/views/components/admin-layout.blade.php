...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-slate-950 font-sans text-slate-100 antialiased">
        ...
        <div class="relative min-h-screen lg:grid lg:grid-cols-[290px_1fr]">
            <aside class="border-b border-white/10 bg-slate-900/80 backdrop-blur-xl lg:border-b-0 lg:border-r">
                <div class="flex h-full flex-col">
                    <div class="border-b border-white/10 px-6 py-6">
                        <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center gap-3">
                            <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-cyan-400/15 text-sm font-semibold tracking-[0.2em] text-cyan-200">
                                EA
                            </div>
                            <div>
                                <p class="text-xs uppercase tracking-[0.3em] text-cyan-200/80">Admin Panel</p>
                    ...
                    <nav class="px-4 py-5">
                        ...
                        <div class="mt-3 space-y-1">
                            <a href="{{ route('admin.dashboard') }}" class="flex items-center rounded-xl px-3 py-2 text-sm transition {{ request()->routeIs('admin.dashboard') ? 'bg-cyan-400/10 font-semibold text-cyan-200' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                                Dashboard
                            </a>
                            <a href="{{ route('admin.events.index') }}" class="flex items-center rounded-xl px-3 py-2 text-sm transition {{ request()->routeIs('admin.events.*') ? 'bg-cyan-400/10 font-semibold text-cyan-200' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                                Events
                            </a>
                            <a href="{{ route('admin.attendance.index') }}" class="flex items-center rounded-xl px-3 py-2 text-sm transition {{ request()->routeIs('admin.attendance.*') ? 'bg-cyan-400/10 font-semibold text-cyan-200' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                                Attendance
                            </a>
                            <a href="{{ route('admin.qr-scanner.index') }}" class="flex items-center rounded-xl px-3 py-2 text-sm transition {{ request()->routeIs('admin.qr-scanner.*') ? 'bg-cyan-400/10 font-semibold text-cyan-200' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                                QR Scanner
                            </a>
                            ...
                            <a href="{{ route('admin.diagnostics.index') }}" class="flex items-center rounded-xl px-3 py-2 text-sm transition {{ request()->routeIs('admin.diagnostics.*') ? 'bg-cyan-400/10 font-semibold text-cyan-200' : 'text-slate-300 hover:bg-white/5 hover:text-white' }}">
                                Diagnostics
                            </a>
                        </div>
                    </nav>
                        ...
                        <div class="rounded-2xl border border-white/10 bg-white/5 p-4">
                            ...
                            <div class="mt-4 flex items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit" class="rounded-lg bg-rose-500/90 px-3 py-2 text-xs font-semibold text-white transition hover:bg-rose-500">
                                        Logout
                                    </button>
                                </form>
                            </div>
            ...
            <main class="p-5 sm:p-8 lg:p-10">
                <header class="mb-8 flex flex-wrap items-end justify-between gap-5">
                    <div>
                        <p class="text-xs uppercase tracking-[0.3em] text-cyan-200/70">Control Center</p>
                ...
                @if (session('status'))
                    <div class="mb-6 rounded-xl border border-emerald-300/20 bg-emerald-400/10 px-4 py-3 text-sm text-emerald-200">
                        {{ session('status') }}
                    </div>
                @endif
