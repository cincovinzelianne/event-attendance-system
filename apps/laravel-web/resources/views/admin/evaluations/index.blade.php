<x-admin-layout title="Feedback & Evaluation Tracking" subtitle="Monitor evaluation completion and connect submissions to event attendance.">
    <div class="mb-6 rounded-2xl border border-white/10 bg-slate-900/70 p-5">
        <form method="POST" action="{{ route('admin.evaluations.seed') }}" class="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-end">
            ...
            <button type="submit" class="rounded-xl bg-cyan-400/90 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300">Seed From Attendance</button>
        </form>
    </div>
                            ...
                            <td class="px-4 py-3">
                                @if ($evaluation->status !== 'submitted')
                                    <form method="POST" action="{{ route('admin.evaluations.mark-submitted', $evaluation) }}">
                                        @csrf
                                        <button type="submit" class="text-cyan-300 transition hover:text-cyan-100">Mark Submitted</button>
                                    </form>
                                @else
