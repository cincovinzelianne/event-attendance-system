<x-admin-layout title="Role & Permission Management" subtitle="Assign advanced capabilities per role for secure module access.">
    <form method="POST" action="{{ route('admin.permissions.update') }}" class="space-y-6">
                            ...
                            <tr>
                                ...
                                @foreach ($roles as $role)
                                    <td class="px-4 py-3 text-slate-300">
                                        <input type="checkbox" name="permissions[{{ $role }}][]" value="{{ $permission->permission_key }}" @checked(in_array($permission->permission_key, $assigned[$role] ?? [], true)) class="rounded border-white/20 bg-white/5 text-cyan-300 focus:ring-cyan-300">
                                    </td>
                                @endforeach
                            </tr>
                        @endforeach
        ...
        <button type="submit" class="rounded-xl bg-gradient-to-r from-cyan-400 to-blue-500 px-4 py-2 text-sm font-semibold text-slate-950 transition hover:-translate-y-0.5">Update Permissions</button>
    </form>
</x-admin-layout>
