...
<div class="grid gap-4 sm:grid-cols-2">
    ...
    <div>
        ...
        <input id="checkin_start_at" name="checkin_start_at" type="datetime-local" value="{{ old('checkin_start_at', isset($event) && $event->checkin_start_at ? $event->checkin_start_at->format('Y-m-d\\TH:i') : '') }}" class="mt-1 block w-full rounded-xl border-white/10 bg-white/5 text-white shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
        @error('checkin_start_at')<p class="mt-1 text-sm text-rose-300">{{ $message }}</p>@enderror
    </div>

    <div>
        <label for="checkin_end_at" class="block text-sm font-medium text-slate-200">Check-in Closes</label>
        <input id="checkin_end_at" name="checkin_end_at" type="datetime-local" value="{{ old('checkin_end_at', isset($event) && $event->checkin_end_at ? $event->checkin_end_at->format('Y-m-d\\TH:i') : '') }}" class="mt-1 block w-full rounded-xl border-white/10 bg-white/5 text-white shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
        @error('checkin_end_at')<p class="mt-1 text-sm text-rose-300">{{ $message }}</p>@enderror
    </div>
    ...
    <div class="sm:col-span-2">
        ...
        <input id="google_form_url" name="google_form_url" type="url" value="{{ old('google_form_url', $event->google_form_url ?? '') }}" class="mt-1 block w-full rounded-xl border-white/10 bg-white/5 text-white shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
        @error('google_form_url')<p class="mt-1 text-sm text-rose-300">{{ $message }}</p>@enderror
    </div>

    <div>
        <label for="target_department" class="block text-sm font-medium text-slate-200">Target Department</label>
        <input id="target_department" name="target_department" type="text" value="{{ old('target_department', $event->target_department ?? '') }}" class="mt-1 block w-full rounded-xl border-white/10 bg-white/5 text-white shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
    ...
    <div>
        ...
        <input id="target_course" name="target_course" type="text" value="{{ old('target_course', $event->target_course ?? '') }}" class="mt-1 block w-full rounded-xl border-white/10 bg-white/5 text-white shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
        @error('target_course')<p class="mt-1 text-sm text-rose-300">{{ $message }}</p>@enderror
    </div>

    <div>
        <label for="target_year_level" class="block text-sm font-medium text-slate-200">Target Year Level</label>
        <input id="target_year_level" name="target_year_level" type="text" value="{{ old('target_year_level', $event->target_year_level ?? '') }}" class="mt-1 block w-full rounded-xl border-white/10 bg-white/5 text-white shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
        @error('target_year_level')<p class="mt-1 text-sm text-rose-300">{{ $message }}</p>@enderror
    </div>

    <div>
        <label for="status" class="block text-sm font-medium text-slate-200">Status</label>
        <select id="status" name="status" class="mt-1 block w-full rounded-xl border-white/10 bg-white/5 text-white shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
    ...
    <div>
        ...
        <input id="poster" name="poster" type="file" accept=".jpg,.jpeg,.png,.webp" class="mt-1 block w-full rounded-xl border-white/10 bg-white/5 text-sm text-slate-200 shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
        @if (! empty($event?->poster_path))
            <p class="mt-1 text-xs text-slate-400">Current: {{ $event->poster_path }}</p>
        @endif
        @error('poster')<p class="mt-1 text-sm text-rose-300">{{ $message }}</p>@enderror
    </div>

    <div>
        <label for="attachment" class="block text-sm font-medium text-slate-200">Attachment File</label>
        <input id="attachment" name="attachment" type="file" class="mt-1 block w-full rounded-xl border-white/10 bg-white/5 text-sm text-slate-200 shadow-sm focus:border-cyan-300 focus:ring-cyan-300">
        @if (! empty($event?->attachment_path))
            <p class="mt-1 text-xs text-slate-400">Current: {{ $event->attachment_path }}</p>
        @endif
        @error('attachment')<p class="mt-1 text-sm text-rose-300">{{ $message }}</p>@enderror
    </div>
